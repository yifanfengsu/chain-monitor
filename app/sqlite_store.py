...
def write_delivery_audit(record: Any) -> bool:
    ...
    ok = _upsert(
        "delivery_audit",
        {
            ...
            "pair": _text(_first(data, "pair", "pair_label", "lp_pair_label")),
            "base": _text(_first(data, "base", "base_symbol", "base_token_symbol")),
            "quote": _text(_first(data, "quote", "quote_symbol", "quote_token_symbol")),
            "side": _text(_first(data, "side", "direction")),
            "intent_type": _text(_first(data, "intent_type", "intent", "canonical_semantic_key")),
            "pool_address": _text(_first(data, "pool_address", "lp_pool_address", "address")),
            "lp_stage": _text(_first(data, "lp_stage", "lp_alert_stage", "stage")),
            "event_ts": _real(_first(data, "event_ts", "timestamp", "ts", "archive_ts")),
            "drop_reason": _text(_first(data, "drop_reason")),
            "drop_metadata_version": _int(_first(data, "drop_metadata_version")),
...
def upsert_opportunity_outcome(record: Any) -> bool:
    ...
    return _upsert(
        "opportunity_outcomes",
        {
            ...
            "evaluated_at": _real(data.get("evaluated_at")),
            "outcome_ts": _real(data.get("outcome_ts")),
            "net_pnl_bps": _real(data.get("net_pnl_bps")),
            ...
            "catchup_reason": _text(data.get("catchup_reason")),
        },
...
def _write_opportunity_outcome_from_record(record: dict[str, Any], opportunity_id: str, window_sec: int, created_at: float) -> bool:
    ...
    return _upsert(
        "opportunity_outcomes",
        {
            ...
            "evaluated_at": _real(record.get(f"outcome_evaluated_at_{key}") or record.get("evaluated_at")),
            "outcome_ts": _real(record.get(f"outcome_ts_{key}") or record.get("outcome_ts")),
            "net_pnl_bps": _real(record.get(f"net_pnl_bps_{key}") or record.get("net_pnl_bps")),
            "created_at": created_at or now,
            "updated_at": now,
            "source": _text(record.get("source")),
            "settled_by": _text(record.get("settled_by")),
            "catchup": _bool_int(record.get("catchup")),
            "catchup_reason": _text(record.get("catchup_reason")),
        },
        ("trade_opportunity_id", "window_sec"),
    )
