...
def parse_command(text: str) -> dict[str, Any]:
    ...
    if match:
        ...
        return {
            "action": "daily-flow",
            "command_intent": "daily-flow",
            "argv": wrapper_argv("daily-flow", "--date", report_date),
            "date": report_date,
            "mode": "",
            "auto_build": False,
        }
    ...
    if text in {"空间检查", "磁盘检查", "VPS空间检查", "数据库空间检查", "DB空间检查"}:
        return {
            "action": "space-check",
            "command_intent": "space-check",
            "argv": wrapper_argv("space-check"),
            "date": "",
            "mode": "",
            "auto_build": False,
        }
    ...
    if match:
        report_date = validate_date(match.group(1))
        return {
            "action": "archive-compress-check",
            "command_intent": "archive-compress-check",
            "argv": wrapper_argv("archive-compress-check", "--date", report_date),
            "date": report_date,
            "mode": "",
            "auto_build": False,
        }
    ...
    if match:
        ...
        return {
            ...
            "command_intent": "weekly-review",
            "argv": wrapper_argv("weekly-review", "--start", start, "--end", end),
            "date": f"{start}..{end}",
            "mode": "",
            "auto_build": False,
        }
